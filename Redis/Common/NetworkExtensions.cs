...
public static class NetworkExtensions
{
    ...
    {
        var resp = RespBuilder.ArrayFromCommands("PING").AsBytes();
        stream.Write(resp);
    ...
    {
        var resp = RespBuilder.ArrayFromCommands("REPLCONF", "listening-port", port.ToString()).AsBytes();
        stream.Write(resp);
    ...
    {
        var resp = RespBuilder.ArrayFromCommands("REPLCONF", "capa", "psync2").AsBytes();
        stream.Write(resp);
    ...
    {
        // e.g. PSYNC ? -1
        var resp = RespBuilder.ArrayFromCommands("PSYNC", "?", "-1").AsBytes();
        stream.Write(resp);
